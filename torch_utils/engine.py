...
def compute_validation_loss(
    ...
    """
    Compute Faster R-CNN validation losses per epoch.

    Torchvision detection models return a loss dict only when:
    1) targets are provided, and
    2) the model is in training mode.

    To log validation losses safely, we temporarily set the model to
    train() under no_grad so weights are not updated.
    """
    was_training = model.training
    model.train()

    metric_logger = utils.MetricLogger(delimiter="  ")
    header = f"Val Loss: [{epoch}]"
    ...
    for images, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if not math.isfinite(loss_value):
            print(f"Validation loss is {loss_value}, stopping training")
            print(loss_dict_reduced)
            sys.exit(1)
