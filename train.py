...
# Training on ResNet50 FPN with custom project folder name ...c augmentation (ON by default) and added training augmentations:
python train.py --model fasterrcnn_resnet50_fpn --epochs 2 ...ug --data data_configs/voc.yaml --name resnet50fpn_voc --batch 4

# Distributed training:
export CUDA_VISIBLE_DEVICES=0,1
python -m torch.distributed.launch --nproc_per_node=2 --use_env ...--model fasterrcnn_resnet50_fpn --name smoke_training --batch 16
"""
"""
USAGE
...
"""
from torch_utils.engine import (
    train_one_epoch, evaluate, compute_validation_loss, utils
)
from torch.utils.data import (
    distributed, RandomSampler, SequentialSampler
)
...
def main(args):
    ...
    # Validation loss lists (epoch-wise means).
    val_loss_list_epoch = []
    val_loss_cls_list = []
    ...
    if args['weights'] is not None:
        ...
        if args['resume_training']:
            ...
            if checkpoint.get('epoch') is not None:
                start_epochs = checkpoint['epoch']
                print(f"Resuming from epoch {start_epochs}...")
            if checkpoint.get('train_loss_list'):
                print('Loading previous batch wise loss list...')
                train_loss_list = checkpoint['train_loss_list']
            if checkpoint.get('train_loss_list_epoch'):
                print('Loading previous epoch wise loss list...')
                train_loss_list_epoch = checkpoint['train_loss_list_epoch']
            if checkpoint.get('val_map'):
                print('Loading previous mAP list')
                val_map = checkpoint['val_map']
            if checkpoint.get('val_map_05'):
                val_map_05 = checkpoint['val_map_05']

            # New (safe) loads for validation losses, if present in older/newer checkpoints.
            if checkpoint.get('val_loss_list_epoch'):
                val_loss_list_epoch = checkpoint['val_loss_list_epoch']
            if checkpoint.get('val_loss_cls_list'):
                val_loss_cls_list = checkpoint['val_loss_cls_list']
            if checkpoint.get('val_loss_box_reg_list'):
                val_loss_box_reg_list = checkpoint['val_loss_box_reg_list']
            if checkpoint.get('val_loss_objectness_list'):
                val_loss_objectness_list = checkpoint['val_loss_objectness_list']
            if checkpoint.get('val_loss_rpn_list'):
                val_loss_rpn_list = checkpoint['val_loss_rpn_list']
    ...
    if args['optimizer'] is not None:
        if args['optimizer'] == 'SGD':
            ...
            # AdamW does not support momentum/nesterov args.
            optimizer = torch.optim.AdamW(
                params,
                lr=args['lr']
            )
        print(f"Using {args['optimizer']} for {args['model']}")
    else:
        if 'dinov3' in args['model']:
            optimizer = torch.optim.AdamW(params, lr=args['lr'])
    ...
    if args['resume_training'] and checkpoint is not None:
        # LOAD THE OPTIMIZER STATE DICTIONARY FROM THE CHECKPOINT.
    ...
    for epoch in range(start_epochs, NUM_EPOCHS):
        ...
        # ---- NEW: Compute validation losses per epoch ----
        _, val_batch_loss_list, \
            val_batch_loss_cls_list, \
            val_batch_loss_box_reg_list, \
            val_batch_loss_objectness_list, \
            val_batch_loss_rpn_list = compute_validation_loss(
            model,
            valid_loader,
            DEVICE,
            epoch,
        ...
        # ---- NEW: Store epoch-wise validation losses (means) ----
        val_loss_list_epoch.append(np.mean(np.array(val_batch_loss_list)))
        val_loss_cls_list.append(np.mean(np.array(val_batch_loss_cls_list)))
        val_loss_box_reg_list.append(np.mean(np.array(val_batch_loss_box_reg_list)))
        val_loss_objectness_list.append(np.mean(np.array(val_batch_loss_objectness_list)))
        val_loss_rpn_list.append(np.mean(np.array(val_batch_loss_rpn_list)))

        # Append current epoch's average loss to `train_loss_list_epoch`.
        train_loss_list_epoch.append(train_loss_hist.value)

        # Validation mAP lists.
        val_map_05.append(stats[1])
        val_map.append(stats[0])
        ...
        # ---- UPDATED CSV LOGGER SIGNATURE ----
        # NOTE: We now pass epoch-wise train loss list and validation loss lists,
        # plus COCO stats will be fully expanded inside logging.py.
        csv_log(
            OUT_DIR,
            stats,
            epoch,
